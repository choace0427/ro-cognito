...
export default function Home() {
  ...
  const handleSignIn = async () => {
    if (signEmail && password) {
      ...
      await axios
        .post(`${process.env.NEXT_PUBLIC_API}/auth/signin`, signinData)
        .then((res) => {
  ...
  const handleForgetPassword = async () => {
    if (email) {
      ...
      await axios
        .post(`${process.env.NEXT_PUBLIC_API}/auth/forgetpassword`, forgetData)
        .then((res) => {
          if (res.data.statusCode === 500) {
            toast.error(res.data.message);
          } else {
  ...
  const handleVerify = async () => {
    if (resetcode) {
      ...
      await axios
        .post(`${process.env.NEXT_PUBLIC_API}/auth/resetpassword`, verificationData)
        .then((res) => {
          if (res.data.statusCode === 500) {
            toast.error(res.data.message);
          } else {
        ...
        <>
          {!reset ? (
            ...
            <Card className='w-96'>
              ...
              <CardBody className='flex flex-col gap-4'>
                <Input label='Verification Code' type='number' size='lg' onChange={(e) => setResetCode(e.target.value)} value={resetcode} inputMode='numeric' />
                <Input label='Reset Password' type='password' size='lg' onChange={(e) => setResetPassword(e.target.value)} value={resetpassword} />
              </CardBody>
              <CardFooter className='pt-0'>
